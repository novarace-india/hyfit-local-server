-- ============================================================================
-- 080: hyfit_v2 — the field-operations schema.
--
-- A fresh schema for the three field apps (Admin, Check-in, Judge). Nothing
-- here is a rename of a `hyfit` table: the old field-ops tables stored the
-- roster and what happened to it, and that job now belongs to RaceResult.
-- What is left is small, and it is what RaceResult cannot hold for us:
--
...
-- feed at the moment it needs them, and writes the result back to the same
-- place. RaceResult carries the hand-over itself — `wristbandid` alongside
-- `wristbandidAssignedBy` and `stage1checkintime` — so a copy here would be a
-- second thing to keep in step. One store, one truth.
--
...
-- row at all, and a constraint would make that state unrepresentable. The
-- athlete platform (`hyfitgames`) and the communication module keep reading
-- `hyfit.events` and `hyfit.users` exactly as they do today; nothing in this
-- file changes anything they can see.
--
-- The traffic in the other direction is deliberate and is exactly one table:
-- `raceresults_endpoints` below is read by the athlete platform's roster
-- importer as well as by the field apps. That is the point of it. The endpoint
-- was already a single shared row in `hyfit.event_configs` — see the comment
-- above `getIntegration` in hfg-setup.service.ts — because two endpoints for
-- one event disagree silently about which roster is real. Moving the field
-- apps here and leaving the importer behind would have recreated that split,
-- so the importer moves too and there is still one row.
--
-- Ordered by dependency: events → endpoints → users → sessions → audit.
...
-- Exactly one live operational event, as in the old schema: two would make
-- "which event is this tablet on?" unanswerable.
...
CREATE TABLE IF NOT EXISTS hyfit_v2.users (
  ...
  -- Which check-in stage this person staffs. Replaces the counters-and-
  -- assignments pair: the stage is a fact about the shift the person is
  -- working, so it lives on the person.
  --
  -- Permitted for admins as well as volunteers, unlike the 078 column it
  -- replaces. An event_admin standing in at a desk is the Help Desk override,
  -- and under 078 they could open a counter but never complete a stage.
  checkin_stage     text
...
COMMENT ON COLUMN hyfit_v2.users.checkin_stage IS
  'Which check-in stage this person staffs. NULL for judges and anyone not on a counter.';
